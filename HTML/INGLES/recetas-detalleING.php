<?php
session_start();
// This page is accessible without a session: recipes marked as public
// can be viewed freely; the rest require a signed-in session (see JS below).
...
<html lang="en">
  <head>
    ...
    <title>Recipe Detail - Canrisk</title>
  ...
  <body>
    ...
    <div class="detalle-wrapper">
      <a
        href="<?php echo $sesion ? 'recetasING.php' : '../../IndexING.php'; ?>"
        class="detalle-volver"
        >&larr; Back to the recipes menu</a
      >
    ...
    <script>
      ...
      if (receta && !receta.publico && !sesionIniciada) {
        document.title = "Recipe requires login - Canrisk";
        contenedor.innerHTML = `
          <div class="receta-bloqueada">
            <div class="icono">🔒</div>
            <h1>${receta.nombre}</h1>
            <p>
              This full recipe is only available to signed-in users.
              Log in or sign up for free to see all the ingredients and
              the step-by-step preparation.
            </p>
            <div class="acciones">
              <a class="btn-login-recetas" href="loginING.php">Log In</a>
              <a class="btn-login-recetas btn-secundario" href="registerING.php">Sign Up</a>
            </div>
          </div>
        `;
      } else if (receta) {
        ...
        contenedor.innerHTML = `
          ...
          <p class="receta-beneficio">
            <strong>Nutritional benefit:</strong> ${receta.beneficio}
          </p>
        ...
        contenedor.innerHTML = `<p style="padding: 40px;">The requested recipe was not found.</p>`;
      }
    </script>
